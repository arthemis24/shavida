...
from movies.views import stream, TestVideoBytesCounter, debit_vod_balance, Home, MediaList, Checkout, MovieDetail, \
    get_media, get_recommended_for_single_category, Search, SeriesDetail, Bundles
from reporting.views import confirm_order
...
urlpatterns = patterns('',
                       ...
                       url(r'^getMovies$', get_media, name='get_movies'),
                       ...
                       url(r'^stream/(?P<item_type>[-\w]+)/(?P<item_id>[-\w]+)/$', stream, name='stream'),
