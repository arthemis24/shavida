...
import os
import sys
# sys.path.append("/home/komsihon/Dropbox/PycharmProjects/Cinemax")
...
from movies.models import Category, Movie, SeriesEpisode, Trailer

PREFIXES = ['cine', 'da', 'xxl', 'comedie', 'concert', 'doc', 'gag', 'oms']
EXTENSION = 'cnmx'

"""
Grab all files in category Top20
Grab all files in category Xcamer
Iterate over all other categories
For each of them:
    grab 2 movies if category is in (actions/adventure, police/thriller, funny or adult)
...
def collect_movies(max_load, unit, base_categories_slugs=[], preferred_categories_slugs=[], exclude_list=[]):
    total_load = 0
    selection = []
    ...
    for category in base_categories:
        for movie in Movie.objects.raw_query({'categories': {'$elemMatch': {'id': ObjectId(category.id)}}, 'visible': True}):
            ...
            try:
                ...
                print "Adding trailer %s: %d%s in all" % (movie.title, total_load, unit_string)
            except Trailer.DoesNotExist:
                pass
            exclude_list.append(movie.id)
    ...
    while previous_total_load < total_load:
        ...
        for category in Category.objects.exclude(slug__in=base_categories_slugs):
            ...
            if category in main_categories:
                movies = Movie.objects.raw_query({'categories': {'$elemMatch': {'id': ObjectId(category.id)}}, 'visible': True}).exclude(pk__in=exclude_list).order_by('-orders', '-release', '-id')[:2]
                for movie in movies:
                    ...
                    exclude_list.append(movie.id)
                    total_load += movie.__dict__[unit_field_name]
                    print "Adding %s. %d movies: %d%s in all" % (movie.title, len(selection), total_load, unit_string)
            else:
                movies = list(Movie.objects.raw_query({'categories': {'$elemMatch': {'id': ObjectId(category.id)}}, 'visible': True}).exclude(pk__in=exclude_list).order_by('-orders', '-release', '-id'))
                if len(movies) >= 1:
                    movie = movies[0]
                    if movie.__dict__[unit_field_name] == 0:
                        exclude_list.append(movie.id)
                    ...
                    if movie.trailer_slug:
                        try:
                            ...
                            selection.append(trailer)
                            total_load += trailer.__dict__[unit_field_name]
                            print "Adding trailer %s: %d%s in all" % (movie.title, total_load, unit_string)
                        except Trailer.DoesNotExist:
                            pass
                    exclude_list.append(movie.id)
...
def collect_series(max_load, unit, exclude_list=[]):
    ...
    while True:
        try:
            episode = SeriesEpisode.objects.all().exclude(series__in=exclude_list).order_by('-orders', '-release')[0]
            ...
            if total_load + series_load > max_load:
                break
            if series.trailer_slug:
                try:
                    trailer = Trailer.objects.get(slug=series.trailer_slug)
                    selection.append(trailer)
                    total_load += trailer.__dict__[unit_field_name]
                    print "Adding trailer %s: %d%s in all" % (series.title, total_load, unit_string)
                except Trailer.DoesNotExist:
                    pass
            ...
            print "Adding %s. %d series: %d%s in all" % (episode.series, len(exclude_list), total_load, unit_string)
        except IndexError:
            break
    return selection
